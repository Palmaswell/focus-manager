...
describe('Keyboard Manager', () => {
  const { getByRole, getByText } = render(
    <List tabIndex={0}>
      ...
      <Item selected={false}>Item Two</Item>
      <Item selected={false}>Item Three</Item>
      <Item selected={false}>Item Four</Item>
      <Item selected={false}>Item Five</Item>
    </List>
  );
  const wrapper = getByRole('listbox');
  const firstItem = getByText('Item One');
  ...
  test('Select first item on keydown', () => {
    fireEvent.keyDown(wrapper, { key: 'ArrowDown', code: 40 });
    console.log(`${wrapper.tabIndex}`);
